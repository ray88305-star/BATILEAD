...
class BatiLeadRealtimeFullStackApp {
  ...
  async initiateGeniusPayPayment() {
    ...
    const buyerEmail = this.currentUser ? this.currentUser.email : `artisan.${cleanPhone}@momo.ci`;
    ...
    // Appel sécurisé au relais backend Vercel Serverless vers GeniusPay CI
    try {
      fetch("/api/pay", {
        method: "POST",
        ...
        console.log("[GeniusPay Serverless Response]:", resp);
        if (resp && resp.checkout_url) {
          this.pendingPaymentData.checkoutUrl = resp.checkout_url;
        }
      }).catch(err => {
        console.log("[GeniusPay Bridge handled]:", err);
      });
    } catch (e) {
