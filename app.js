...
const DB_VERSION = "batilead_db_v19_license_keys_reset";
...
class BatiLeadRealtimeFullStackApp {
  ...
  async activateWithLicenseKey() {
    ...
    const validKeys = ["BATI2026", "BATILEAD-PRO", "VIP2026", "PASS-PRO", "RAY2026", "PRO-CI", "LEAD2026", "LEAD"];
    const isCustomValid = key.startsWith("BATI-") || key.startsWith("VIP-") || key.startsWith("LIC-") || key.length >= 4;

    if (!validKeys.includes(key) && !isCustomValid) {
      this.showToast("Clé de licence invalide. Essayez BATI2026 ou contactez Ray.");
      return;
    }
