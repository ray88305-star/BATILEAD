 ...
 * Synchronisation Temps Réel Supabase Cloud + Local Cache
 */

const NOW = Date.now();
const ONE_DAY_MS = 1000 * 60 * 60 * 24;
const DB_VERSION = "batilead_db_v12_fullstack_supabase";
...
class BatiLeadFullStackApp {
  ...
  forceDirectDatabaseUpdate() {
    try {
      ...
      if (currentVersion !== DB_VERSION) {
        ...
        localStorage.setItem("batilead_geniuspay_leads_db_v12", JSON.stringify(DEFAULT_LEADS_CI));
        localStorage.setItem("batilead_master_admin_email_v12", "ray88305@gmail.com");
        localStorage.setItem("batilead_geniuspay_pubkey_v12", "sk_sandbox_kU48PXqvgWojG0mWRc7EaKsgIf5DlC1E");
        localStorage.setItem("batilead_geniuspay_secret_v12", "ss_sandbox_YMvxy8Q5UnLAY3T9hWJ1oiZtMQAC4bHSisv5BUhoTzwwNyf5");
        localStorage.setItem("batilead_geniuspay_mode_v12", "sandbox");
  ...
  // Synchronisation descendante : Récupère les leads de Supabase
  ...
  // Synchronisation montante : Envoie tous les leads locaux vers Supabase
  ...
  getOrCreateDeviceId() {
    let device = localStorage.getItem("batilead_device_fingerprint_v12");
    if (!device) {
      device = `197.234.${Math.floor(10 + Math.random() * 80)}.${Math.floor(10 + Math.random() * 80)}`;
      localStorage.setItem("batilead_device_fingerprint_v12", device);
    }
    return device;
  }

  loadAdminEmail() {
    return localStorage.getItem("batilead_master_admin_email_v12") || "ray88305@gmail.com";
  }

  saveAdminEmail(email) {
    this.adminEmail = email.trim().toLowerCase();
    localStorage.setItem("batilead_master_admin_email_v12", this.adminEmail);
  ...
  loadGeniusPayKey() {
    return localStorage.getItem("batilead_geniuspay_pubkey_v12") || "sk_sandbox_kU48PXqvgWojG0mWRc7EaKsgIf5DlC1E";
  }

  loadGeniusPaySecret() {
    return localStorage.getItem("batilead_geniuspay_secret_v12") || "ss_sandbox_YMvxy8Q5UnLAY3T9hWJ1oiZtMQAC4bHSisv5BUhoTzwwNyf5";
  }

  loadGeniusPayMode() {
    return localStorage.getItem("batilead_geniuspay_mode_v12") || "sandbox";
  }
  ...
  saveGeniusPayConfig(key, secret, mode) {
    ...
    localStorage.setItem("batilead_geniuspay_pubkey_v12", this.geniusPayPubKey);
    localStorage.setItem("batilead_geniuspay_secret_v12", this.geniusPaySecretKey);
    localStorage.setItem("batilead_geniuspay_mode_v12", this.geniusPayMode);
  }

  loadCurrentUser() {
    const saved = localStorage.getItem("batilead_current_user_v12");
    if (saved) {
  ...
  saveCurrentUser(user) {
    ...
    if (user) {
      localStorage.setItem("batilead_current_user_v12", JSON.stringify(user));
      this.registerOrUpdateUserInDb(user);
    } else {
      localStorage.removeItem("batilead_current_user_v12");
    }
    this.updateAuthUi();
  }

  loadLeads() {
    const saved = localStorage.getItem("batilead_geniuspay_leads_db_v12");
    if (saved) {
      try {
  ...
  saveLeads() {
    this.processLeadPricing();
    localStorage.setItem("batilead_geniuspay_leads_db_v12", JSON.stringify(this.leads));
    this.updateCategoryCounts();
    this.updateNavCounts();
    this.renderAdminFullStack();
  }
  ...
  loadUsersRegistry() {
    const saved = localStorage.getItem("batilead_users_registry_v12");
    if (saved) {
  ...
  saveUsersRegistry() {
    localStorage.setItem("batilead_users_registry_v12", JSON.stringify(this.users));
  }
  ...
  loadOrdersRegistry() {
    const saved = localStorage.getItem("batilead_orders_registry_v12");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return []; }
    }
    return [];
  ...
  saveOrdersRegistry() {
    localStorage.setItem("batilead_orders_registry_v12", JSON.stringify(this.orders));
  }
  ...
  getDeviceUnlockedLeadIds() {
    const saved = localStorage.getItem("batilead_unlocked_lead_ids_v12");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return []; }
    }
    return [];
  ...
  saveDeviceUnlockedLeadId(leadId) {
    ...
    if (!ids.includes(leadId)) {
      ids.push(leadId);
      localStorage.setItem("batilead_unlocked_lead_ids_v12", JSON.stringify(ids));
    }
  }
