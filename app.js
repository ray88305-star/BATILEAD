...
class BatiLeadRealtimeFullStackApp {
  ...
  async handleAuthSubmit(event) {
    ...
    // 1. Enregistrement / Connexion Cloud Supabase
    if (this.supabaseClient && this.supabaseClient.auth) {
      try {
        ...
        console.warn("Supabase Auth handled:", e);
      }
    }

    // 2. Création et validation immédiate de la session utilisateur
    const user = {
      id: `USR-${Date.now()}`,
      name: name,
      email: email,
    ...
    if (submitBtn) {
      ...
      submitBtn.textContent = this.authMode === "signup" ? "Créer mon Compte" : "Se Connecter";
    }
    ...
    // Si l'utilisateur voulait débloquer un chantier précis, on lui ouvre directement le guichet de paiement
      ...
      container.innerHTML = `
        <div class="flex items-center gap-2.5 bg-sky-50 border border-sky-200/80 px-3.5 py-1.5 rounded-2xl cursor-pointer hover:shadow-sm transition-all" onclick="app.navigateTo('crm')">
          <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-sky-500 to-sky-600 text-white flex items-center justify-center font-bold text-xs shadow-sm">
            ${this.currentUser.name ? this.currentUser.name.charAt(0).toUpperCase() : 'B'}
          </div>
          <div class="text-left hidden sm:block">
            <span class="text-slate-900 font-bold text-xs block leading-tight">${this.currentUser.name || 'Mon Compte Bati\'O'}</span>
