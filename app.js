...
class BatiLeadRealtimeFullStackApp {
  ...
  forceDirectDatabaseUpdate() {
    try {
      ...
      if (currentVersion !== DB_VERSION) {
        ...
        localStorage.setItem("batilead_geniuspay_pubkey_v18", "sk_sandbox_kU48PXqvgWojG0mWRc7EaKsgIf5DlC1E");
        localStorage.setItem("batilead_geniuspay_secret_v18", "ss_sandbox_YMvxy8Q5UnLAY3T9hWJ1oiZtMQAC4bHSisv5BUhoTzwwNyf5");
        localStorage.setItem("batilead_geniuspay_mode_v18", "sandbox");
    ...
    this.supabaseClient.auth.onAuthStateChange((event, session) => {
      ...
      if (session && session.user) {
        ...
        const authedUser = {
          ...
          email: email,
          phone: phone,
          role: this.isSuperAdmin(email) ? "super_admin" : "artisan",
          avatar: user.user_metadata?.avatar_url || "",
          loggedAt: new Date().toISOString()
        };

        this.saveCurrentUser(authedUser);
        this.logActivity("AUTH_STATE_CHANGE", `Session active pour ${email} (via ${user.app_metadata?.provider || 'Supabase'})`);
      }
    });
  }
  ...
  setAuthMode(mode) {
    ...
    const fieldPhone = document.getElementById("auth-field-phone");
    ...
    if (mode === "signup") {
      ...
      if (submitBtn) submitBtn.textContent = "Créer mon Compte Pro";
    } else {
  ...
  async signInWithRealGoogleOAuth() {
    this.logActivity("GOOGLE_OAUTH_CLICK", "Redirection vers Google Sign-In officiel (accounts.google.com)");
    if (!this.supabaseClient) this.initSupabaseClient();
    if (!this.supabaseClient || !this.supabaseClient.auth) {
      this.showToast("Service Supabase en cours d'initialisation...");
      return;
    }

    try {
      this.showToast("Connexion à Google en cours...");
      ...
      console.warn("Google OAuth Note:", e);
      if (e.message && e.message.toLowerCase().includes("provider is not enabled")) {
        alert("⚠️ Pour activer le vrai bouton Google OAuth :\n\n1. Rendez-vous sur votre console Supabase : Authentication > Providers > Google\n2. Activez 'Google' (Enable Google Provider)\n3. Collez vos identifiants Google Cloud (Client ID & Secret).");
      } else {
        this.showToast(`Google OAuth : ${e.message}`);
      }
    }
  }
  ...
  async handleAuthSubmit(event) {
    ...
    if (submitBtn) {
      ...
      submitBtn.innerHTML = `<span class="animate-spin mr-1">⏳</span> Authentification...`;
    }

    const isAdmin = this.isSuperAdmin(email);

    // 1. Appel Supabase Auth Réel & Vérification Stricte
    if (this.supabaseClient && this.supabaseClient.auth) {
      try {
        if (this.authMode === "signup") {
          ...
          if (error) {
            if (submitBtn) {
              ...
              submitBtn.textContent = "Créer mon Compte Pro";
            }
            if (error.message.toLowerCase().includes("already registered")) {
              this.showToast("❌ Cet email existe déjà. Veuillez cliquer sur 'Se Connecter'.");
              this.setAuthMode("login");
            } else {
              this.showToast(`❌ Erreur d'inscription Supabase : ${error.message}`);
            }
            return;
          }

          this.showToast(`✅ Compte créé dans Supabase Cloud pour ${name} !`);
        } else {
          ...
          if (error) {
            ...
            if (error.message.toLowerCase().includes("invalid login credentials")) {
              this.showToast("❌ Email ou mot de passe incorrect. Si vous débutez, cliquez sur 'Créer un Compte'.");
            } else {
              this.showToast(`❌ Erreur de connexion Supabase : ${error.message}`);
            }
            return;
          }

          this.showToast(`✅ Connexion Supabase Cloud réussie !`);
        }
      } catch (e) {
    ...
    // 2. Profil utilisateur validé et mémorisé
    const user = {
      name: name,
      email: email,
      phone: phone,
    ...
    if (submitBtn) {
      submitBtn.disabled = false;
      submitBtn.textContent = this.authMode === "signup" ? "Créer mon Compte Pro" : "Se Connecter";
    }

    this.logActivity("LOGIN", `Connexion vérifiée pour ${name} (${email})`);
  ...
  quickAdminLogin() {
    ...
    this.saveCurrentUser(user);
    this.closeModal("authModal");
    this.logActivity("LOGIN", "Connexion Super-Admin Express (ray88305@gmail.com)");
  ...
  updateAuthUi() {
    ...
    if (this.currentUser) {
      container.innerHTML = `
        <div class="flex items-center gap-2 bg-amber-50/80 border border-amber-200/80 px-3 py-1.5 rounded-2xl cursor-pointer hover:shadow-sm transition-all" onclick="app.navigateTo('crm')">
          <div class="w-7 h-7 rounded-xl bg-amber-600 text-white flex items-center justify-center font-bold text-xs">
            ${this.currentUser.name ? this.currentUser.name.charAt(0).toUpperCase() : 'G'}
          </div>
          <div class="text-left hidden sm:block">
            <span class="text-slate-900 font-bold text-xs block leading-none">${this.currentUser.name || 'Compte Google'}</span>
            <span class="text-[10px] text-emerald-700 font-semibold font-mono">${this.currentUser.email}</span>
          </div>
  ...
  getDeviceUnlockedLeadIds() {
    const saved = localStorage.getItem("batilead_unlocked_lead_ids_v18");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return []; }
    }
    return [];
  ...
  saveDeviceUnlockedLeadId(leadId) {
    ...
    if (!ids.includes(leadId)) {
      ids.push(leadId);
      localStorage.setItem("batilead_unlocked_lead_ids_v18", JSON.stringify(ids));
    }
  }
  ...
  getMaskedName(fullName, isPurchased) {
    ...
    return `M. ${first} ${initial}•••• (Client Vérifié)`;
  }

  getMaskedPhone(phone, isPurchased) {
    if (isPurchased) return phone;
    const lastDigits = phone ? phone.slice(-2) : "77";
    return `+225 07 •• •• •• ${lastDigits}`;
  }

  getMaskedEmail(email, isPurchased) {
    if (isPurchased) return email;
    const parts = email.split("@");
    return `${parts[0].substring(0, 3)}••••@client.ci`;
  }
  ...
  createHumanLeadCardHtml(lead) {
    ...
    return `
      <div class="soft-lead-card p-5 sm:p-6 flex flex-col justify-between ${isPurchased ? 'border-sky-300 bg-sky-50/20' : 'bg-white'}">
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-start justify-between gap-3 pb-3 border-b border-slate-100">
            <div>
              <div class="flex items-center gap-2">
                ...
                ${isPurchased ? '<span class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">Débloqué Bati\'O</span>' : ''}
              </div>
          ...
          <div class="space-y-2.5">
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="text-[10px] font-bold px-2.5 py-0.5 rounded-full shrink-0 ${lead.category === 'construction' ? 'bg-sky-50 text-sky-900 border border-sky-200/80' : 'bg-blue-50 text-blue-900 border border-blue-200/80'}">
                ${lead.categoryLabel}
              </span>
            </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex items-center justify-between gap-3 mt-4">
          ...
          <!-- Right: Action Button -->
          ${isPurchased ? `
            <button class="btn-sky-secondary text-xs py-2 px-3.5" onclick="app.openLeadDetailModal('${lead.id}')">
              <i data-lucide="eye" class="w-3.5 h-3.5 text-sky-600"></i> Voir les coordonnées
            </button>
          ` : isExpired ? `
  ...
  renderCrmBoard() {
    ...
    const purchased = this.leads.filter(l => l.status === "purchased" && (
      (l.unlockedByEmail && l.unlockedByEmail.toLowerCase() === userEmail) ||
      (l.createdBy && l.createdBy.toLowerCase() === userEmail) ||
      this.getDeviceUnlockedLeadIds().includes(l.id)
    ));
