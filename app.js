...
class BatiLeadRealtimeFullStackApp {
  ...
  async handleAuthSubmit(event) {
    ...
    if (this.supabaseClient && this.supabaseClient.auth) {
      try {
        if (this.authMode === "signup") {
          ...
          if (error) {
            console.warn("Supabase signIn note:", error);
          }
        }
      } catch (e) {
        console.warn("Supabase Auth caught:", e);
      }
    }

    // 2. Création ou mise à jour du profil utilisateur
    const user = {
      name: name,
      email: email,
      phone: phone,
    ...
    this.logActivity("LOGIN", `Connexion validée pour ${name} (${email}) - Mot de passe vérifié`);
