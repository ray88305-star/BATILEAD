/**
 * BatiLead Pro — Moteur Full-Stack & Synchronisation Multi-Appareils en Temps Réel
 * Base de Données Partagée Supabase Cloud (PostgreSQL)
 * URL : https://ozxenmrmaomaqzkyjobc.supabase.co
 * Key : sb_publishable_iaTe3CardO3ESuXZzqtNgg_sDGjbUmR
 * Passerelle GeniusPay Mobile Money (Wave, Orange, MTN, Moov)
 */

const NOW = Date.now();
const ONE_DAY_MS = 1000 * 60 * 60 * 24;
const DB_VERSION = "batilead_db_v15_supabase_connected";

const OFFICIAL_SUPABASE_URL = "https://ozxenmrmaomaqzkyjobc.supabase.co";
const OFFICIAL_SUPABASE_KEY = "sb_publishable_iaTe3CardO3ESuXZzqtNgg_sDGjbUmR";
...
class BatiLeadRealtimeFullStackApp {
  constructor() {
    ...
    // Supabase Cloud Config (Official Credentials hardcoded & pre-configured)
    this.supabaseUrl = OFFICIAL_SUPABASE_URL;
    this.supabaseKey = OFFICIAL_SUPABASE_KEY;
  ...
  forceDirectDatabaseUpdate() {
    try {
      ...
      if (currentVersion !== DB_VERSION) {
        ...
        localStorage.setItem("batilead_geniuspay_leads_db_v15", JSON.stringify(DEFAULT_LEADS_CI));
        localStorage.setItem("batilead_master_admin_email_v15", "ray88305@gmail.com");
        localStorage.setItem("batilead_geniuspay_pubkey_v15", "sk_sandbox_kU48PXqvgWojG0mWRc7EaKsgIf5DlC1E");
        localStorage.setItem("batilead_geniuspay_secret_v15", "ss_sandbox_YMvxy8Q5UnLAY3T9hWJ1oiZtMQAC4bHSisv5BUhoTzwwNyf5");
        localStorage.setItem("batilead_geniuspay_mode_v15", "sandbox");
  ...
  initSupabaseClient() {
    if (window.supabase && this.supabaseUrl && this.supabaseKey) {
      try {
        ...
        console.log("Supabase Client Initialized with Project ozxenmrmaomaqzkyjobc.");
        this.fetchLeadsFromSupabaseCloud();
      } catch (e) {
  ...
  updateSupabaseStatusBadge(isConnected) {
    ...
    if (isConnected) {
      ...
      badge.innerHTML = `<span class="w-2 h-2 rounded-full bg-amber-500"></span> Connexion Supabase...`;
    }
  }
  ...
  async testSupabaseConnection() {
    ...
    try {
      this.showToast("Connexion à Supabase Cloud (ozxenmrmaomaqzkyjobc) en cours...");
  ...
  loadActivityLogs() {
    const saved = localStorage.getItem("batilead_activity_logs_v15");
    if (saved) {
  ...
  saveActivityLogs() {
    localStorage.setItem("batilead_activity_logs_v15", JSON.stringify(this.activityLogs));
  ...
  getOrCreateDeviceId() {
    let device = localStorage.getItem("batilead_device_fingerprint_v15");
    if (!device) {
      device = `197.234.${Math.floor(10 + Math.random() * 80)}.${Math.floor(10 + Math.random() * 80)}`;
      localStorage.setItem("batilead_device_fingerprint_v15", device);
    }
    return device;
  }

  loadAdminEmail() {
    return localStorage.getItem("batilead_master_admin_email_v15") || "ray88305@gmail.com";
  }

  saveAdminEmail(email) {
    this.adminEmail = email.trim().toLowerCase();
    localStorage.setItem("batilead_master_admin_email_v15", this.adminEmail);
  ...
  loadGeniusPayKey() {
    return localStorage.getItem("batilead_geniuspay_pubkey_v15") || "sk_sandbox_kU48PXqvgWojG0mWRc7EaKsgIf5DlC1E";
  }

  loadGeniusPaySecret() {
    return localStorage.getItem("batilead_geniuspay_secret_v15") || "ss_sandbox_YMvxy8Q5UnLAY3T9hWJ1oiZtMQAC4bHSisv5BUhoTzwwNyf5";
  }

  loadGeniusPayMode() {
    return localStorage.getItem("batilead_geniuspay_mode_v15") || "sandbox";
  }
  ...
  saveGeniusPayConfig(key, secret, mode) {
    ...
    localStorage.setItem("batilead_geniuspay_pubkey_v15", this.geniusPayPubKey);
    localStorage.setItem("batilead_geniuspay_secret_v15", this.geniusPaySecretKey);
    localStorage.setItem("batilead_geniuspay_mode_v15", this.geniusPayMode);
  }

  loadCurrentUser() {
    const saved = localStorage.getItem("batilead_current_user_v15");
    if (saved) {
  ...
  saveCurrentUser(user) {
    ...
    if (user) {
      localStorage.setItem("batilead_current_user_v15", JSON.stringify(user));
      this.registerOrUpdateUserInDb(user);
    } else {
      localStorage.removeItem("batilead_current_user_v15");
    }
    this.updateAuthUi();
  }

  loadLeads() {
    const saved = localStorage.getItem("batilead_geniuspay_leads_db_v15");
    if (saved) {
      try {
  ...
  saveLeads() {
    this.processLeadPricing();
    localStorage.setItem("batilead_geniuspay_leads_db_v15", JSON.stringify(this.leads));
    this.updateCategoryCounts();
    this.updateNavCounts();
    this.renderAdminFullStack();
  }
  ...
  loadUsersRegistry() {
    const saved = localStorage.getItem("batilead_users_registry_v15");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return []; }
    }
    return [
  ...
  saveUsersRegistry() {
    localStorage.setItem("batilead_users_registry_v15", JSON.stringify(this.users));
  }
  ...
  loadOrdersRegistry() {
    const saved = localStorage.getItem("batilead_orders_registry_v15");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return []; }
    }
    return [];
  ...
  saveOrdersRegistry() {
    localStorage.setItem("batilead_orders_registry_v15", JSON.stringify(this.orders));
  }
  ...
  getDeviceUnlockedLeadIds() {
    const saved = localStorage.getItem("batilead_unlocked_lead_ids_v15");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return []; }
    }
    return [];
  ...
  saveDeviceUnlockedLeadId(leadId) {
    ...
    if (!ids.includes(leadId)) {
      ids.push(leadId);
      localStorage.setItem("batilead_unlocked_lead_ids_v15", JSON.stringify(ids));
    }
  }
  ...
  handleManualLeadSubmit(event) {
    ...
    // Supabase Cloud Sync immédiat vers tous les téléphones et PC
    if (this.supabaseClient) {
