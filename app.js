...
class BatiLeadRealtimeFullStackApp {
  ...
  async handleGoogleCredentialResponse(response) {
    ...
    try {
      // Décodage du vrai jeton Google sécurisé
      ...
      console.log("Vrai compte Google authentifié :", googleUser);

      const isRay = this.isSuperAdmin(googleUser.email);
      const user = {
        id: googleUser.sub || `USR-GGL-${Date.now()}`,
        name: googleUser.name || googleUser.given_name || "Utilisateur Google",
      ...
      // 1. Sauvegarde locale de la session
      ...
      if (this.supabaseClient) {
        ...
        await this.supabaseClient.from("user_sessions_log").insert({
          ...
          details: `Connexion réelle via compte Google ${user.email} enregistrée dans Supabase Cloud`
        }).catch(console.warn);
      }

      this.closeModal("authModal");
      this.logActivity("LOGIN_GOOGLE", `Connexion officielle Google réussie pour ${user.name} (${user.email})`);
      if (window.confetti) confetti({ particleCount: 60, spread: 70, origin: { y: 0.6 } });

      if (isRay) {
        this.showToast(`👑 Bienvenue Ray ! Accès Gestion & Ajout de chantiers activé.`);
      } else {
        this.showToast(`✅ Bienvenue ${user.name} ! Connecté avec Google.`);
      }
      this.updateAuthUi();
      ...
      console.error("Erreur décodage compte Google:", e);
      this.showToast("Erreur lors de la connexion Google.");
    }
  }
  ...
  async signInWithRealGoogleOAuth() {
    this.logActivity("GOOGLE_OAUTH_CLICK", "Ouverture du sélecteur officiel de comptes Google (prompt select_account)");
    if (!this.supabaseClient) this.initSupabaseClient();
    if (!this.supabaseClient || !this.supabaseClient.auth) {
    ...
    try {
      this.showToast("Ouverture de Google en cours...");
      const isHttp = window.location.protocol.startsWith("http");
      const redirectUrl = isHttp 
        ? window.location.origin + window.location.pathname 
        : "https://batilead-nu.vercel.app";
